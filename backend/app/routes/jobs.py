"""Jobs API routes — with AI-powered query generation and scoring."""
import json
import logging
...
def update_job_status(job_id):
    """Update a job's status and record preference signal."""
...
def refresh_jobs():
    """
    Full pipeline: AI Queries → Fetch → Blacklist → Deduplicate → Score → AI Score
    """
    ...
    try:
        # ── Step 0: AI generates additional search queries ──
        ai_query_count = 0
        try:
            ...
            if is_ai_enabled():
                ...
                if ai_queries:
                    ...
                    # Store AI queries for the fetcher to use
                    current_app.config["_ai_queries"] = ai_queries
        except Exception as e:
            logger.info(f"AI query generation skipped: {e}")

        # ── Step 1: Fetch jobs from all sources (includes AI queries) ──
        ...
        # ── Step 4: Score remaining jobs (rule-based) ──
        scored = score_all_jobs(profile)

        # ── Step 5: AI re-scores top jobs with explanations ──
        ai_scored = 0
        try:
            from app.services.ai_agent import ai_score_jobs, is_ai_enabled
            if is_ai_enabled():
                # Get top 16 new jobs by base score
                ...
                if top_jobs:
                    ...
                    # Update jobs with AI scores and reasons
                    if ai_results:
                        with get_connection() as conn:
                            for job_id, ai_data in ai_results.items():
                                ai_score = ai_data["ai_score"]
                                ai_reason = ai_data["ai_reason"]
                                # Blend: 60% rule-based + 40% AI score
                                base = conn.execute(
                                ...
                                if base:
                                    ...
                                    conn.execute(
                                        """UPDATE jobs 
                                           SET adjusted_score = ?, ai_score = ?, ai_reason = ?
                                           WHERE id = ?""",
                                        (blended, ai_score, ai_reason, job_id)
                                    )
                            conn.commit()
