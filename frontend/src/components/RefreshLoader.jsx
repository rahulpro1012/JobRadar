...
export default function RefreshLoader({ profile }) {
  ...
  useEffect(() => {
    const stageTimer = setInterval(() => {
      setStageIndex((prev) => {
        if (prev >= waitStart) {
          return waitStart + ((prev - waitStart + 1) % 5);
        }
        return prev + 1;
      });
    }, 3500);
    ...
    <div className="flex flex-col items-center justify-center py-16 animate-fade-in">
      <div className="relative mb-8">
        <div className="w-20 h-20 rounded-2xl bg-brand-600/20 flex items-center justify-center radar-pulse">
          <Radar className="w-10 h-10 text-brand-400 animate-radar-sweep" />
        </div>
        <div className="absolute inset-0 w-20 h-20">
          ...
          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-2 w-1.5 h-1.5 rounded-full bg-accent-500 animate-pulse-slow" />
          <div className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 w-1.5 h-1.5 rounded-full bg-brand-300 animate-pulse" />
        </div>
      </div>
      <h3 className="font-display font-semibold text-lg text-surface-100 mb-3">
        Finding your next role{dots}
      </h3>
      <div className="flex items-center gap-3 mb-5 min-h-[28px]">
        <span className="badge bg-brand-500/15 text-brand-400 border border-brand-500/25 text-xs min-w-[70px] justify-center">
          {stage.phase}
        </span>
        <p
          className="text-sm text-surface-400 animate-fade-in"
          key={stageIndex}
        >
          {stage.text}
        </p>
      </div>
      <div className="w-72 h-1.5 bg-surface-800 rounded-full overflow-hidden mb-3">
        <div
          className="h-full bg-gradient-to-r from-brand-600 via-brand-400 to-brand-500 rounded-full transition-all duration-1000 ease-out"
