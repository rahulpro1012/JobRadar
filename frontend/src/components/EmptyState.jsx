...
export default function EmptyState({ type, onAction }) {
  const states = {
    noProfile: {
      icon: Upload, title: 'Upload your resume to get started',
    ...
    noJobs: {
      icon: Search, title: 'No jobs found yet',
    ...
    noResults: {
      icon: Radar, title: 'No jobs match current filters',
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4 animate-fade-in">
      ...
      <h3 className="font-display font-semibold text-lg t-primary mb-2 text-center">{s.title}</h3>
      <p className="text-sm t-muted text-center max-w-md mb-6">{s.description}</p>
      {s.actionLabel && onAction && (<button onClick={onAction} className="btn-primary">{s.actionLabel}</button>)}
    </div>
  );
}
